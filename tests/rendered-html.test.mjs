...
test("keeps resume content centralized in the data file", async () => {
  const [data, page, hero, skills, experience, projects, spec] =
    await Promise.all([
  ...
  assert.match(hero, /resume\.personal/);
  assert.match(skills, /resume\.skillGroups/);
  assert.match(experience, /resume\.experience/);
  assert.match(projects, /resume\.projects/);
